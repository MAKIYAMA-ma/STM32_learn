...
{
   msTicks++;
  // if((msTicks%SYSTICK_HZ)==0)wizpf_putchar('+',WIZ_USART1);
}
...
int8_t platform_init(usart_param *up)
{
	...
	xSysCtlDelay(20000000);

	xSysTickPeriodSet(xSysCtlClockGet()/2000);
	...
	xGPIODirModeSet(LED1_PORT, LED1_PIN,xGPIO_DIR_MODE_OUT);
	xGPIODirModeSet(LED2_PORT, LED2_PIN,xGPIO_DIR_MODE_OUT);
...
int8_t wizpf_spi_init(wizpf_spi spi)
{
	switch(spi) {
	   ...
	   xSPIConfigSet(xSPI0_BASE, 20000000, xSPI_MOTO_FORMAT_MODE_0 | xSPI_MODE_MASTER | xSPI_MSB_FIRST | xSPI_DATA_WIDTH8);

	   xHWREG(xSPI0_BASE+SPI_CNTRL) |= SPI_CNTRL_VARCLK_EN;
	...
	case WIZ_SPI2:
	   ...
	   xSPIConfigSet(xSPI1_BASE, 20000000, xSPI_MOTO_FORMAT_MODE_0 | xSPI_MODE_MASTER | xSPI_MSB_FIRST | xSPI_DATA_WIDTH8);
