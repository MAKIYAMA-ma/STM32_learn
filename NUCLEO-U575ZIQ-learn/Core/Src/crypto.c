                     ...
                     uint8_t *signature, size_t sig_len)
{
    ...
    // DEBUG
    log_printf(DBG_LVL_DBG, "HASH:\n");
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < 8; j++) {
            log_printf(DBG_LVL_DBG, "%02X ", hash[i*8+j]);
        }
        log_printf(DBG_LVL_DBG, "\n");
    }
    ...
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < 8; j++) {
            log_printf(DBG_LVL_DBG, "%02X ", sp[i*8+j]);
        }
        log_printf(DBG_LVL_DBG, "\n");
    }
...
void TestVerifySignature(void)
{
    ...
    if(ret == 0) {
        log_printf(DBG_LVL_DBG, "Verify Signature Suceess!\n");
    } else {
        log_printf(DBG_LVL_DBG, "Verify Signature Fail!\n");
    }
}
